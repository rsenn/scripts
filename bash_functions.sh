...
";  unset ARGS;
 ...
 grep-in-index() {
  (CMD='index-dir -u $DIRS | xargs grep "[^/]\$" -H | sed "s|/files.list:|/|" -u | xargs grep -H -E "($EXPRS)" '
   case "$PATHTOOL" in
   ...
   while :; do
     case "$1" in
       ...
       --basic-regexp|--binary|--byte-offset|--count|--dereference-recursive|--extended-regexp|--files-with-matches|--files-without-match|--fixed-strings|--help|--ignore-case|--initial-tab|--invert-match|--line-buffered|--line-number|--line-regexp|--no-filename|--no-messages|--null|--null-data|--only-matching|--perl-regexp|--quiet|--recursive|--silent|--text|--unix-byte-offsets|--version|--with-filename|--word-regexp) GREP_ARGS="${GREP_ARGS:+$GREP_ARGS }$1"; shift ;;
       *) break ;;
     esac
    done
