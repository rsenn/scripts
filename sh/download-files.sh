...
trap ': ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} --color -H ".*" "$COOKIEFILE"; rm -f "$COOKIEFILE"
: ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} --color -H ".*" "$HEADERFILE"; rm -f "$HEADERFILE"
...
#  ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} -E -i "\.($EXTLIST)\$" |
#  while read FILE; do
    ...
    *)
     (#set -x
      curl -s \
        ...
        #${SED-sed} -e 's,[-+a-z]\+://,\n&,g' | ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} '^[-+a-z]*://' | ${SED-sed} -e 's,[ ">].*,,' 
    ;;
  esac |  #|
 (grep -E -i "\.($EXTLIST)\$") |
 (while read FILE; do
