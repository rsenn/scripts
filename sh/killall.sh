...
if [ "$OS" = Darwin ]; then
...
elif (ps --help; ps -X -Y -Z) 2>&1 | ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} -q '\-W'; then
  PS="ps"
  PSARGS="-aW"
  ...
  PS="ps"
  if (ps --help; ps -X -Y -Z) 2>&1 | ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} -q '\-W'; then
    PSARGS="-aW"
  else
    PSARGS="axw"	
  fi
...
case `uname -o 2>/dev/null || uname` in
  ...
  *Linux*) ;;
  *)
  if type kill.exe 2>/dev/null >/dev/null && (kill.exe --help 2>&1 | ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} -q '\-f.*win32'); then
    KILL="kill.exe"
    KILLARGS="${KILLARGS:+
}-f"
...
PSMATCH=` echo "$PSOUT" | ([ "$DEBUG" = true ] && set -x; ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} -i -E "$PATTERN" ) |grep -v -E "(killall\\.sh|\\s$$\\s)"`
