...
for DLL in *.dll; do
 (LIBNAME=${DLL%.dll}
  ...
  IMPLIB=`ls -d ../lib/lib${ANAME}*.dll.a 2>/dev/null | ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} "/lib/lib${ANAME}[-_0-9]*\.dll.a\$"`
  MSIMPLIB=`ls -d ../lib/${ANAME}*.lib 2>/dev/null | ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} "/lib/${ANAME}[-_0-9]*\.lib\$"`
