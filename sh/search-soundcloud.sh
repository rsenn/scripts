...
case "$OS" in
 ...
 # LINKS=`(set -x; curl --location  "$URL") | ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} -E '<(h3|div class="pagination")>'  | xml_get a href`
  LINKS=`extract-urls.sh "$URL"`
  echo LINKS = "$LINKS" 1>&2
	TRACKS=`echo "$LINKS" | ${GREP-grep${NL}-a${NL}--line-buffered${NL}--color=auto} -v page=`
