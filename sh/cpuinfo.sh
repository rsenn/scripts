...
-a
--line-buffered
--color=auto} "^model name.* \<Pentium(R) 2\>" $CPUINFO > /dev/null; then
    MARCH="pentium2"; CFLAGS="$CFLAGS -march=$MARCH";
elif ${GREP-grep
-a
--line-buffered
--color=auto} "^model name.* \<Pentium(R) 3\>" $CPUINFO > /dev/null; then
    MARCH="pentium3"; CFLAGS="$CFLAGS -march=$MARCH";
elif ${GREP-grep
-a
--line-buffered
--color=auto} "^model name.* \<Pentium(R) 4\>" $CPUINFO > /dev/null; then
    MARCH="pentium4"; CFLAGS="$CFLAGS -march=$MARCH";
elif ${GREP-grep
-a
--line-buffered
--color=auto} "^model name.* \<AMD Athlon(tm) 64\>" $CPUINFO > /dev/null; then
    MARCH="athlon64"; CFLAGS="$CFLAGS -march=$MARCH";
elif ${GREP-grep
-a
--line-buffered
--color=auto} "^model name.* \<AMD Phenom(tm) II\>" $CPUINFO > /dev/null; then
...
-a
--line-buffered
--color=auto} "^flags.* mmx\>" $CPUINFO > /dev/null; then
    USE="$USE mmx"
...
-a
--line-buffered
--color=auto} "^flags.* mmx2\>" $CPUINFO > /dev/null; then
    USE="$USE mmx2"
...
-a
--line-buffered
--color=auto} "^flags.* mmxext\>" $CPUINFO > /dev/null; then
    USE="$USE mmmxext"
    # TODO: Upcoming version of GCC may add this flag
...
-a
--line-buffered
--color=auto} "^flags.* sse\>" $CPUINFO > /dev/null; then
    USE="$USE sse"
...
-a
--line-buffered
--color=auto} "^flags.* sse2\>" $CPUINFO > /dev/null; then
    USE="$USE sse2"
...
-a
--line-buffered
--color=auto} "^flags.* ssse3\>" $CPUINFO > /dev/null; then
    USE="$USE sse3"
...
-a
--line-buffered
--color=auto} "^flags.* sse4\>" $CPUINFO > /dev/null; then
    USE="$USE sse4"
...
-a
--line-buffered
--color=auto} "^flags.* sse4.1\>" $CPUINFO > /dev/null; then
    USE="$USE sse4.1"
...
-a
--line-buffered
--color=auto} "^flags.* sse4.2\>" $CPUINFO > /dev/null; then
    USE="$USE sse4.2"
...
-a
--line-buffered
--color=auto} "^flags.* abm\>" $CPUINFO > /dev/null; then
    USE="$USE abm"
...
-a
--line-buffered
--color=auto} "^flags.* sse4a\>" $CPUINFO > /dev/null; then
    USE="$USE sse4a"
...
-a
--line-buffered
--color=auto} "^flags.* sse5\>" $CPUINFO > /dev/null; then
    USE="$USE sse5"
...
-a
--line-buffered
--color=auto} "^flags.* 3dnow\>" $CPUINFO > /dev/null; then
    USE="$USE 3dnow"
...
-a
--line-buffered
--color=auto} "^flags.* 3dnowext\>" $CPUINFO > /dev/null; then
    USE="$USE 3dnowext"
    # TODO: Upcoming version of GCC may add this flag
