...
MARCH=native;
if grep "^model name.* \<Pentium(R) 2\>" $CPUINFO > /dev/null; then
    MARCH="pentium2"; CFLAGS="$CFLAGS -march=$MARCH";
elif grep "^model name.* \<Pentium(R) 3\>" $CPUINFO > /dev/null; then
    MARCH="pentium3"; CFLAGS="$CFLAGS -march=$MARCH";
elif grep "^model name.* \<Pentium(R) 4\>" $CPUINFO > /dev/null; then
    MARCH="pentium4"; CFLAGS="$CFLAGS -march=$MARCH";
elif grep "^model name.* \<AMD Athlon(tm) 64\>" $CPUINFO > /dev/null; then
    MARCH="athlon64"; CFLAGS="$CFLAGS -march=$MARCH";
elif grep "^model name.* \<AMD Phenom(tm) II\>" $CPUINFO > /dev/null; then
    MARCH="amdfam10"; CFLAGS="$CFLAGS -march=$MARCH";
fi

if grep "^flags.* mmx\>" $CPUINFO > /dev/null; then
    USE="$USE mmx"
    CMFLAGS="$CMFLAGS -mmmx"
    CONF_FLAGS="$CONF_FLAGS --enable-mmx"
fi

if grep "^flags.* mmx2\>" $CPUINFO > /dev/null; then
    USE="$USE mmx2"
...
if grep "^flags.* mmxext\>" $CPUINFO > /dev/null; then
    USE="$USE mmmxext"
    # TODO: Upcoming version of GCC may add this flag
...
if grep "^flags.* 3dnowext\>" $CPUINFO > /dev/null; then
    USE="$USE 3dnowext"
    # TODO: Upcoming version of GCC may add this flag
