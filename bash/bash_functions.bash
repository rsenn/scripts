...
bitrate()
{
  ...
  do
    ...
    R=0
    set -- $(mminfo "$ARG"  |${SED-sed} -n "/[Oo]verall [bB]it [Rr]ate\s*=/ { s,\s*Kbps\$,, ;  s|\([0-9]\)\s\+\([0-9]\)||g ; s,\.[0-9]*\$,, ; s,^[^=]*=,,; s|^|$ARG:|; p }")
...
cut-ver()
{
 (while :; do
  ...
  ${SED-sed} 's,-\([0-9]\+\):\([0-9]\+\),-.,' |
  ${SED-sed} 's,[.-][[:digit:]][_+[:alnum:]~]*$,,g ;; s,[.-][[:digit:]][_+[:alnum:]~]*\([-.]\),,g'|
  ${SED-sed} 's,[-_.][[:digit:]]*\(svn\)\?\(git\)\?\(P\)\?\(rc\)\?[[:digit:]][_+[:digit:]]*\(-.\),,g' |
...
get-tempdir() {
 (TEMPDIR=
  if type reg 2>/dev/null >/dev/null; then
    TEMPDIR=`reg query 'HKCU\Environment' '/v' TEMP | ${SED-sed} -n 's|.*REG_SZ\s\+\(.*\)||p'`
    [ -d "$TEMPDIR" ] || TEMPDIR=
  fi
  if [ -z "$TEMPDIR" ]; then
...
vcget() {
  ...
  BINDIR="$VCINSTALLDIRin${ARCH:+\$ARCH}"
  CL="$BINDIR