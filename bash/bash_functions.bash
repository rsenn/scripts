...
"
  ...
  while :; do case "$1" in
      -x|--debug) DEBUG=true
        shift ;;
      -0|--null|-a|--all|--apparent-size|-c|--total|-D|--dereference-args|--summarize|-H|--dereference-args|-h|--human-readable|--inodes|-L|--dereference|-l|--count-links|-P|--no-dereference|-S|--separate-dirs|--si|-h|-s|--summarize|--time|-x|--one-file-system|--help|--version|--block-size) pushv DU_ARGS "$1"
        shift ;;
      -B=*|-b=*|-d=*|-k=*|-m=*|-t=*|-X=*) pushv DU_ARGS "${1%%=*}" "${1#-?=}"
        shift ;;
      -B|-b|-d|-k|-m|-t|-X) pushv DU_ARGS "$1" "$2"
        shift 2 ;;
      -B*|-b*|-d*|-k*|-m*|-t*|-X*) A=${1#-?}
        pushv DU_ARGS "${1%%$A}" "${A}"
        shift ;;
      --block-size=*|--exclude-from=*|--exclude=*|--files0-from=*|--max-depth=*|--threshold=*|--time-style=*|--time=*) pushv DU_ARGS "$1"
        shift ;;
      --block-size|--exclude|--exclude-from|--files0-from|--max-depth|--threshold|--time|--time-style) pushv DU_ARGS "$1=$2"
        shift 2 ;;
      *) break ;;
    esac; done
  echo -n >${TMP}
  ...
  if [ -w "${TMP}" ]; then
    CMD="${CMD} | (tee \"\$TMP\"; sort -n -k1 <\"\$TMP\" >$OUT; rm -f \"\$TMP\"; echo \"Saved list into $OUT\" 1>&2)"
  fi
  
  [ "${DEBUG}" = true ] && echo "+ $CMD" >&2
  eval "${CMD}")
}
...
gen-move-ebooks() {
  for F in "$@"; do BASEDIR=$(\
echo "${F}" | sed "s|\(.*Books[^/]*\)/.*|\1|i ; s|\(.*Calibre[^/]*\)/.*|\1|i"); RELPATH=${F#$BASEDIR/}; \
      echo "mkdir -p 'G:/Books/${RELPATH%/*}'; mv -vf '$F' 'G:/Books/${RELPATH%/*}'"; done
}
...
get-eagle-file() {
  tasklist /fi "IMAGENAME eq eagle*" /v /fo list 2>&1 | sed -n \
  's,\\,/,g; s,\r*$,,; /Window Title:/ s,.* - \(.*\) - EAGLE.*,\1,p'
}

get-ebooks() {
  export \
      DATABASE=$(cygpath -am "${USERPROFILE}/AppData/Roaming/Locate32/files.dbs"); ls $LS_ARGS -td -- $((locate32.sh -E{pdf,epub,mobi,azw3,djv,djvu} | grep -i -E '(books|calibre)' \ find-media.sh -E{pdf,epub,mobi,azw3,djv,djvu} calibre books) | \
  sort \
  -f \
  -u)
}
...
make-playlists() {
  VIDEOS=; DATABASE=$(ls -d -- "$(cygpath -am "${USERPROFILE}")"/AppData/*/Locate32/*.dbs | \
  filter-filesize \
  -gt \
  1k); \
    ...
    wc -l <${VIDEOS} >&2; \
    msg "Acquiring videos using find \$(list-mediapath ...)"; pushv VIDEOS "$(for_each -f 'find "$1" -type f -not -name "*.part"' \
  $(list-mediapath -m {,Downloads/}{Videos/,Porn/}) | grep-videos.sh)"; \
    wc -l <${VIDEOS} >&2; \
    msg "Merging videos..."; VIDEOS=$(ls -td -- \
  $(realpath $(sed 's,\r*$,, ; s,\\\+,/,g' <${VIDEOS} | filter-test -e)) 2>/dev/null | \
  sed \
  's,^/cygdrive,, ; s,^/\(.\)/\(.*\),\1:/\2,' | \
  sort \
  -f \
  ...
  15M); \
    set -- $VIDEOS; \
    msg "Acquired $# videos."; split_results() {
      ...
      for LL in videos porn; do LN=$LL-by-$NAME; msg "Writing $LN.m3u"; \
          eval 'make-m3u.sh $(<'$LN'.list) |sed "s|/|\\\\|g ; s|\\r*\$|\\r|" >'$LN'.m3u'; done
}; for CMD in "ls -"{t,S}"d --"; do case "${CMD}" in
        *-S*) NAME=size ;;
        *-t*) NAME=time ;;
      esac; \
      eval 'R=$('$CMD' "$@" 2>/dev/null)'; \
      split_results; \
      write_playlist; done; for CMD in "duration -m" "duration" "bitrate" "resolution"; do \
      NAME=${CMD-//" "/""}; \
      OUT=$NAME.tmp; \
      EVAL="${CMD} \"\$@\" >$OUT"; \
      msg "Executing: $EVAL"; \
...
mkzroot() {
  ...
  for DIR in $DEST; do (CMD="cp --remove-destination -vf  \"\$TEMPTXZ\"  \"\$DIR/zroot.xz\""
  eval "echo \"+ $CMD\" 1>&2"
  eval "${CMD}"); done)
}
...
sndfile-duration() {
  for ARG in "$@"; do I=$(sndfile-info "${ARG}" | sed 's,^Duration[: ]*,,p' -n); \
      echo "$ARG${SEP--|}${I}"; done
}
...
triplet-to-arch() {
  (for TRIPLET; do OS=; BITS=; TRIPLET=${TRIPLET##*/}; (case "${TRIPLET}" in
    *x86?64*|*x64*|*amd64*) BITS=64 ;;
    *i[3-8]86*|*x32*|*x86*) BITS=32 ;;
  esac
  OS=${TRIPLET%-gnu}
  OS=${OS##*-}
  OS=${OS%32}
...
win-get-environment() {
  ...
  while :; do case "$1" in
      ...
      -s|--separator) S=$2
        shift 2 ;;
      -g|--global|--local*machine*) GLOBAL=true
        shift ;;
      *) break ;;
    esac; done
  ...
  EXPR="/REG_SZ/ { $EXPR; p }"
  [ "${GLOBAL}" = true ] && \
KEY='HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Environment' || KEY='HKCU\Environment'
  [ $# -le 0 ] && set -- PATH
  for VAR; do reg query "${KEY}" /v "${VAR}"; done | sed -n "${EXPR}")
}
